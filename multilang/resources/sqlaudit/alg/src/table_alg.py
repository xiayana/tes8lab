...
class alg_detection(base):
    """
    混合算法检测
    1.判定cmd是否出现在【cmdset】中
    2.基于实时传递的实时统计量和【cmdstatic】判定异常
    3.基于fre概率计算余弦相似度
    4.基于【cmdsrc】判定对应的搜索条件是否正常
    """    
    ...
    def detection(self, user, windowlist, model,userdf):
        ...
        for item in windowlist:
            cmd_list.append([item['time'],item['cmd'],item['src']])
            if item['cmd'] not in cal_data['cmdset']:
                res += "Have ill cmd with %s in %s; " %(item['cmd'],user)
            else:
                if item['src'] not in cal_data['cmd_src'][item['cmd']]:
                    res += "Have ill condition with %s in %s; " %(item['src'],user)
             
                
        #cmd_list.sort() #当前算法不需排序
        ...
        if user!='null' and len(windowlist)/self.winsize>0.15: #当检测窗口数据量过小,不进行异常系数计算
            #  计算词袋的字典概率分布
            ...
            simlog="CMD概率分布相似度 %f，CMD NG相似度 %f，Table概率分布相似度 %f, TableNG相似度 %f, 综合异常评判系数 %f; " %(simularity_dic_cmd,\
                                                                                                                simularity_cmd_ngram,\
                                                                                                                simularity_dic_table,\
                                                                                                                simularity_table_ngram,\
                                                                                                                execsim)
            '''
            execsim= simularity_dic_cmd
            simlog="CMD 概率分布相似度 %f; " %simularity_dic_cmd
