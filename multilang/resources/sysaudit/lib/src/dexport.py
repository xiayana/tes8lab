...
class export_redis:
    ...
    def __init__(self,conf,log):
        ...
        try:
            ...
            #     _cluster_conf = conf['rediscluster']
            #     # _cluster_conf = [{'host': '192.168.1.241', 'port': 6379}, {'host': '192.168.1.241', 'port': 6380}]
            #     log.debug('-----cluster conf %s' %(_cluster_conf))
            #     from rediscluster import RedisCluster
            #     self.rds = RedisCluster(startup_nodes=_cluster_conf, password=self.rpassword)
            # else:
            import redis
            if self.security == 'on':
