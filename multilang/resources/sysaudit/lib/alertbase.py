...
class alert(object):
  ...
  def setCmd(self,cmd):
    self.alert[0]['alertContext']['properties']['cmd']=cmd

  def setSrc(self,src):
    self.alert[0]['alertContext']['properties']['src']=src

  def setReason(self,reason):
    self.alert[0]['alertContext']['properties']['reason']=reason

  def setPolicyId(self,policyId):
    self.alert[0]['alertContext']['properties']['policyId']=policyId

  def setDst(self,dst):
    self.alert[0]['alertContext']['properties']['dst']=dst

  def delPolicyId(self):
    del self.alert[0]['alertContext']['properties']['policyId']

  def getAlertTimestamp(self):
    return self.alert[0]['alertContext']['properties']['alertTimestamp']
  ...
  def setAlertevent(self,event): #python3 base64
    ...
    # d=json.dumps(event, ensure_ascii=False)
    # d=event
