...
import static org.apache.storm.kafka.spout.FirstPollOffsetStrategy.*;

public class TriasKafkaSpout<K, V> extends KafkaSpout<K, V> implements Serializable {
    ...
    private transient TriasKafkaSpout.KafkaSpoutConsumerRebalanceListener rebalanceListener;

    private transient int loc = 0;

    // 光大资源配置特殊处理，故意跳过部分tuple，如果配置为5，那么只保留5分之1到消费端，扔到5分之4
    public void setSkipPercent(int skipPercent) {
        if (skipPercent ==0){
            this.skipPercent = 10;
            LOG.error("get skipPercent from config is 0,so set to 10");
            return;
        }
        this.skipPercent = skipPercent;
    }

    // 光大资源配置特殊处理，故意跳过部分tuple，比如配置为5，那么只保留5分之1到消费端，扔到5分之4
    private int skipPercent = 0;
    public TriasKafkaSpout(KafkaSpoutConfig<K, V> kafkaSpoutConfig, ConsumerFactory<K, V> kafkaConsumerFactory, TopicAssigner topicAssigner) {
    ...
    public void open(Map<String, Object> conf, TopologyContext context, SpoutOutputCollector collector) {
        ...
        rebalanceListener = new TriasKafkaSpout.KafkaSpoutConsumerRebalanceListener();
    ...
    private class KafkaSpoutConsumerRebalanceListener implements ConsumerRebalanceListener {
        ...
        private long doSeek(TopicPartition newTp, OffsetAndMetadata committedOffset) {
            LOG.info("Seeking offset for topic-partition [{}] with [{}] and committed offset [{}]",
            ...
            if (committedOffset != null) {
                ...
                if (commitMetadataManager.isOffsetCommittedByThisTopology(newTp,
                    ...
                    consumer.seek(newTp, committedOffset.offset()*2);
                } else {
    ...
    public void nextTuple() {
        try {
            ...
            if (commitTimer != null && commitTimer.isExpiredResetOnTrue()) {
                if (isAtLeastOnceProcessing()) {
                    commitOffsetsForAckedTuples();
                } else if (kafkaSpoutConfig.getProcessingGuarantee() == KafkaSpoutConfig.ProcessingGuarantee.NO_GUARANTEE) {
            ...
            TriasKafkaSpout.PollablePartitionsInfo pollablePartitionsInfo = getPollablePartitionsInfo();
    ...
    private TriasKafkaSpout.PollablePartitionsInfo getPollablePartitionsInfo() {
        if (isWaitingToEmit()) {
            LOG.debug("Not polling. Tuples waiting to be emitted.");
            return new TriasKafkaSpout.PollablePartitionsInfo(Collections.emptySet(), Collections.emptyMap());
        }

        Set<TopicPartition> assignment = consumer.assignment();
        if (!isAtLeastOnceProcessing()) {
            return new TriasKafkaSpout.PollablePartitionsInfo(assignment, Collections.emptyMap());
        }
        ...
        if (this.skipPercent == 0){
            this.skipPercent = 10;
            LOG.error("the skipPercent 0,so set to 10");
        }
        for (TopicPartition tp : assignment) {
            loc++;
            if (loc==Integer.MAX_VALUE ){
                LOG.info("the loc is MAX_INT,so changge to 0");
                loc =0;
            }
            ...
            if (numUncommittedOffsets < maxUncommittedOffsets) {
                ...
                // 光大资源配置特殊处理，故意跳过部分tuple，如果配置为5，那么只保留5分之1到消费端，扔到5分之4
                if (loc % skipPercent >0) {
                    LOG.info("the loc is [{}],the yushu is[{}]",loc,(loc % skipPercent));
                    continue;
                }
...
//                            numUncommittedOffsets, maxUncommittedOffsets);
                    LOG.info("Not polling on partition [{}]. It has [{}] uncommitted offsets, which exceeds the limit of [{}]. ", tp,
                            numUncommittedOffsets, maxUncommittedOffsets);
                }
            }
        }
        return new TriasKafkaSpout.PollablePartitionsInfo(pollablePartitions, earliestRetriableOffsets);
    }
    ...
    // ======== poll =========
    private ConsumerRecords<K, V> pollKafkaBroker(TriasKafkaSpout.PollablePartitionsInfo pollablePartitionsInfo) {
    ...
    public void ack(Object messageId) {
        ...
        LOG.info("ths msg is " + msgId.toString());
//        long set = msgId.offset();
//        msgId.
////        msgId.
        tupleListener.onAck(msgId);
    }
    ...
    public Map<String, Object> getComponentConfiguration() {
        ...
        for (Map.Entry<String, Object> conf : kafkaSpoutConfig.getKafkaProps().entrySet()) {
