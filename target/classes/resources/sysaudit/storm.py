...
def sendMsgToParent(msg):#python3 debug
    #import json
    #with open(r'/tmp/sysnisa/msg.txt','a') as f:
        #f.write('keys')
        #for i in msg.keys():
            #try:
...
def emitBolt(tup, stream=None, anchors = [], directTask=None):
    ...
    if stream is not None:
        m["stream"] = stream
    m["anchors"] = list([a.id for a in anchors])
...
class BasicBolt(object):
    ...
    def getstormargs(self):
        conf,context=initComponent()
        self.conf=conf
        self.context=context
        
    def getaskid(self):
        return str(self.context['taskid'])

    def run(self):
        global MODE
        MODE = Bolt
        global ANCHOR_TUPLE
        conf, context = self.conf,self.context
        try:
            self.initialize(conf, context)
            while True:
                tup = readTuple()
