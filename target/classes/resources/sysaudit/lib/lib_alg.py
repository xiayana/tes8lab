...
class malware(object):
    ...
    def setflagwithrs(self,tup,knowdata):
        ...
        if 'type' in knowdata:
            ...
            keys=[tup['cmd']]
        self.log.debug(f"读取的key是：{keys},knowdata:{knowdata}")
        for k in keys:
    ...
    def checkcmdwithknowledge_act(self,tup,know):
        if tup['cmd'] not in know:
            self.log.debug(f"{tup['cmd']} not in know")
            return False
        alist=tup['src'].split('+')
        cellknow=know[tup['cmd']]
        self.log.debug(f"checkcmdwithknowledge_act-->{cellknow}---cmd:{tup['cmd']}\n")
        ...
        if 'content' in cellknow:
            ...
            self.log.debug(f"into setflagwithrs for cellknow:{cellknow}")
            self.setflagwithrs(tup, cellknow)
            return True
