...
class opert_mysql:
    ...
    def getusermodel(self,log,account,sence,whiltlist=[],blacklist=[],mulit=['core','dl','service']):
        alg = ''
        if len(whiltlist) == 1:
            if whiltlist[0] == 'service':
                alg = 'service'
        if blacklist:
            log.warn(f"getusermodel: {str(blacklist)}")
            models={}
            return models
        with self.lock:
            if alg != '':
                self.cursor.execute("select id from %s where sence='%s' and alg='%s'" % (self.table, sence, alg))
            else:
        ...
        for i in ids:
            ...
            try:
                ...
                log.warn(f"mode decode error: detail: {e}")
                log.warn(f"id error:{i}")
                continue
            else:
                log.warn(f"id:{i}")
            ...
            if name in mulit: #当算法为指定算法时，支持多模型投票模型结构，即用户画像数据结构同一用户同一算法可以有多个模型
                if type(model['data'])==dict:
                    ...
                    log.error("NotVaildModelData : the err model  " + str(model))
                    mobj = None
